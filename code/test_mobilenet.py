...
# -*- coding: utf-8 -*-
"""
Test MobileNet v2 on HAM10000 with saved weights.

特点：
1. 按训练脚本（ResNet/MobileNet）的逻辑重建数据划分：
   - lesion_id 去重
   - 从 undup 中抽 20% 作为 val 基底
   - 用 image_id 标记 train / val
   - 训练集 = train_or_val == 'train'
   - 测试集 = train_or_val == 'val'（不再二次切半）
2. 测试集保持原始不均衡分布，但每一类的样本数量比之前多。
3. 评估内容：
   - Overall accuracy
...
def initialize_mobilenet(num_classes, feature_extract=False, use_pretrained=False):
    """
    构建与训练时相同结构的 MobileNet v2。
    """
...
def main():
    ...
    # 假设 test_mobilenet.py 在 code/ 目录下，项目根目录是上一级
    CODE_DIR = os.path.dirname(os.path.abspath(__file__))
    BASE_DIR = os.path.dirname(CODE_DIR)
    PIC2_DIR = os.path.join(BASE_DIR, "pic2")
    ensure_dir(PIC2_DIR)

    weight_path = os.path.join(BASE_DIR, "best_mobilenet_ham10000.pth")
    if not os.path.exists(weight_path):
        raise FileNotFoundError(
            f"未找到模型权重文件：{weight_path}\n"
            f"请先在训练脚本中使用 torch.save(model.state_dict(), 'best_mobilenet_ham10000.pth') 保存。"
        )
    ...
    # -------- 5. 构建 MobileNet v2 并加载权重 --------
    model = initialize_mobilenet(num_classes=num_classes,
    ...
    print("\n=================== MobileNet Test Report (Imbalanced Test Set) ===================")
    ...
    report_path = os.path.join(PIC2_DIR, "mobilenet_classification_report.txt")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Test set classification report (MobileNet v2, imbalanced test set)\n")
    ...
    plot_confusion_matrix(
        ...
        normalize=False,
        title='Test Confusion Matrix (MobileNet v2, Imbalanced Test Set)'
    )
    cm_path = os.path.join(PIC2_DIR, "mobilenet_confusion_matrix.png")
    plt.tight_layout()
    ...
    plt.title('Multi-class ROC Curves (MobileNet v2, Imbalanced Test Set)')
    plt.legend(loc='lower right')
    plt.tight_layout()
    roc_path = os.path.join(PIC2_DIR, "mobilenet_roc_curves.png")
    ...
    plt.title('Multi-class Precision-Recall Curves (MobileNet v2, Imbalanced Test Set)')
    plt.legend(loc='lower left')
    plt.tight_layout()
    pr_path = os.path.join(PIC2_DIR, "mobilenet_pr_curves.png")
    plt.savefig(pr_path, dpi=300)
    plt.show()
    print(f"[SAVED] Precision-Recall curves -> {pr_path}")

    # -------- 11. 结束提示 --------
    print("\n=================== Finished (MobileNet v2 Test) ===================")
...
if __name__ == "__main__":
    print(">>> This is test_mobilenet.py")
